#This GPT Predictor - Uses Extra Docs + Relevant Fields
...
def GPT_1_promptFormat(claim_data):
    return [
        {"role": "system", "content": SYSTEM_PROMPT.strip()},
        {"role": "user", "content": json.dumps(claim_data)}
    ]

def GPT_1_Call(claim_data):
    response = client.chat.completions.create(
        model=GPT_1_jobid,
        messages=GPT_1_promptFormat(claim_data)
    )
    return response.choices[0].message.content

def GPT_1_predictor(claim_data, GPT_1_Call, retries=5):
...
def process_sample(claim_data_sample):
    true_val, pred_val = GPT_1_predictor(claim_data_sample, GPT_1_Call)
    return true_val, pred_val
...
jsonfilepath="/Users/vishnucharan/Downloads/insurance/GPT_Model/WithDocs/TestData/Insurance_data_test_withDocs.jsonl"
main(jsonfilepath)